...
/* TO IMPLEMENT YOUR VERSION OF THE DRIVER YOU'LL HAVE TO EDIT THIS SECTION ONLY */
	...
	#define LCD_MOSI_HIGH()         GPIOA -> ODR |= GPIO_PIN_1
	#define LCD_MOSI_LOW()          GPIOA -> ODR &= ~GPIO_PIN_1

	#define LCD_SCK_HIGH()          GPIOA -> ODR |= GPIO_PIN_0
	#define LCD_SCK_LOW()           GPIOA -> ODR &= ~GPIO_PIN_0
...
const unsigned char FontLookup [][5] =
{
    ...
    { 0x55, 0x2A, 0x55, 0x2A, 0x55}, // Malla cuadrada en posicion de daigonal invertida
	...
	{ 0x00, 0x0E, 0x0A, 0x0E, 0x00 } // Dibuja símbolo de grados en la posicion de la tilde (~)
